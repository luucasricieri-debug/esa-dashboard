...
import type { KpiDelta } from '@/lib/demo';
...
export function Modal({
  ...
  return (
    <div
      className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[10100] grid place-items-center p-3 md:p-6"
      onClick={onClose}
    >
      <div
...
export function Field({ label, hint, children, colSpan }: { label: string; hint?: string; children: ReactNode; colSpan?: 1 | 2 | 3 }) {
...
export function Drawer({
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[10050]" onClick={onClose}>
      <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" />
      <div
        onClick={(e) => e.stopPropagation()}
        className="absolute right-0 inset-y-0 w-full sm:max-w-lg bg-white border-l border-slate-200 shadow-xl flex flex-col"
      >
        <div className="flex items-start justify-between gap-4 px-5 py-4 border-b border-slate-100">
          <div>
            <h3 className="text-base font-semibold text-slate-900">{title}</h3>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      {desc && <div className="text-sm text-slate-500 mt-1 max-w-xs">{desc}</div>}
...
export function SuccessBanner({
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
      <div className="h-5 w-5 rounded-full bg-emerald-500 grid place-items-center text-white text-xs shrink-0 mt-0.5">
        ✓
      </div>
      <div className="flex-1">
        <div className="text-sm font-semibold text-emerald-900">{title}</div>
        {desc && <div className="text-xs text-emerald-700 mt-0.5">{desc}</div>}
      </div>
      {onDismiss && (
        <button onClick={onDismiss} className="text-emerald-500 hover:text-emerald-700 text-lg leading-none">
          ×
        </button>
      )}
    </div>
...
export function ErrorBanner({
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3">
      <div className="h-5 w-5 rounded-full bg-rose-500 grid place-items-center text-white text-xs shrink-0 mt-0.5">
        !
      </div>
      <div className="flex-1">
        <div className="text-sm font-semibold text-rose-900">{title}</div>
        {desc && <div className="text-xs text-rose-700 mt-0.5">{desc}</div>}
      </div>
      {onDismiss && (
        <button onClick={onDismiss} className="text-rose-400 hover:text-rose-600 text-lg leading-none">
          ×
        </button>
      )}
    </div>
