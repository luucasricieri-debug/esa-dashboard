...
import type { KpiDelta } from "@/lib/esa/provider";
...
export function Card({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-slate-200 bg-white shadow-sm shadow-slate-100/60",
        className,
      )}
    >
      {children}
...
export function KpiCard({
  ...
  deltaLabel?: string;
  onClick?: () => void;
  /** For metrics where "down" is good (e.g. alerts, repasse). */
  invertDelta?: boolean;
}) {
  const toneMap = {
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          <div
            className={cn(
              "mt-1.5 md:mt-2 text-xl md:text-2xl font-semibold tabular-nums truncate",
              toneMap[tone],
            )}
          >
            {value}
      ...
      {steps.map((label, i) => {
        ...
        return (
          <li key={label} className="flex items-center gap-1 md:gap-2 shrink-0">
            ...
            <span
              className={cn(
                "text-[11px] md:text-xs whitespace-nowrap",
            ...
            {i < steps.length - 1 && (
              <span className="h-px w-4 md:w-6 bg-slate-200 mx-1" />
            )}
          </li>
        );
      })}
