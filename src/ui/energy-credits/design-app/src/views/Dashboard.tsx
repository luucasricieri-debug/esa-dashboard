...
import { Card, KpiCard, SectionTitle, Badge, Button, CycleBadge, StatusPill, FieldRow } from '@/components/ui/index';
import { demoProvider, type SettlementResult, type Alert } from '@/lib/demo';
...
export function Dashboard({ onNavigate }: { onNavigate: (v: ViewKey) => void }) {
  const months = demoProvider.listMonths();
  ...
  const summary = useMemo(() => demoProvider.getExecutiveSummary(filters), [month, ugFilter]);
  const alertsList = useMemo(() => demoProvider.getAlertsSummary(filters), [month, ugFilter]);
  const trend = useMemo(() => demoProvider.getMonthlyTrend({ ugId: filters.ugId }), [ugFilter]);
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <select
              ...
              className="h-9 min-w-[180px] border border-slate-200 rounded-lg px-3 text-sm bg-white outline-none focus:border-emerald-500"
            >
              {months.map((m) => (
              ...
              {chartMode === 'por_ug' ? (
                <BarChart data={chartGenVsCons}>
                  ...
                  <Tooltip
                    ...
                    labelFormatter={(l, payload) => (payload?.[0]?.payload as { fullName?: string } | undefined)?.fullName ?? l}
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-2.5">
            {alertsList.length === 0 && (
              <div className="text-center text-xs text-slate-500 py-6">Nenhum alerta neste mês</div>
            )}
      ...
      {/* Drill-down Drawer */}
      {drill && (
        <div className="fixed inset-0 z-[10050]" onClick={() => setDrill(null)}>
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" />
          <div
            className="absolute right-0 inset-y-0 w-full sm:max-w-lg bg-white border-l border-slate-200 shadow-xl flex flex-col overflow-y-auto"
...
function DrillContent({
  ...
  if (drill.kind === 'compensated' || drill.kind === 'generation' || drill.kind === 'balance') {
    ...
    return (
      <>
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-slate-900">{titleMap[drill.kind]}</h2>
          <p className="text-xs text-slate-500">Composição por unidade geradora — {monthLabel}</p>
        </div>
      ...
      <>
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-slate-900">
            {isRevenue ? 'Receita ESA' : 'Repasse ao proprietário'}
          </h2>
          <p className="text-xs text-slate-500">
            {isRevenue ? `Detalhamento por beneficiária — ${monthLabel}` : `Detalhamento por UG — ${monthLabel}`}
          </p>
        </div>
        <div className="space-y-2">
          {isRevenue
    ...
    return (
      <>
        <div className="mb-6 flex items-center gap-2">
          <h2 className="text-lg font-semibold text-slate-900">{r.ug.name}</h2>
        </div>
  ...
  if (drill.kind === 'ub') {
    ...
    return (
      <>
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-slate-900">{row.ub.name}</h2>
          <p className="text-xs text-slate-500">{row.ub.id} · vinculada a {ug.name}</p>
        </div>
        <div className="grid grid-cols-2 gap-2">
    ...
    return (
      <>
        <div className="mb-6 flex items-center gap-2">
          ...
          <span className="uppercase text-xs text-slate-600">{a.severity}</span>
        </div>
        <div className="space-y-3">
