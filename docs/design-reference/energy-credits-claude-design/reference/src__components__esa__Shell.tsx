...
const titles: Record<ViewKey, { title: string; sub: string }> = {
  dashboard: {
    title: "Visão Geral",
    sub: "Panorama executivo da operação de créditos de energia",
  },
...
export function Shell() {
  ...
  useEffect(() => {
    return onNavigate((d) => setView(d.view as EsaView as ViewKey));
  }, []);
  ...
  return (
    <div className="flex min-h-screen bg-[#f6f8f6] text-slate-800">
      ...
      <aside
        className={cn(
          "hidden md:flex flex-col border-r border-slate-200 bg-white transition-[width] duration-200 relative",
          collapsed ? "w-16" : "w-60",
        )}
      >
