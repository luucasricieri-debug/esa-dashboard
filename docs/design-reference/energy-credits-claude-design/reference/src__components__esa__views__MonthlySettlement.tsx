...
import { generatingUnits, months } from "@/lib/esa/mockData";
import { energyCreditsProvider } from "@/lib/esa/provider";
...
export function MonthlySettlement() {
  ...
  const [cyclePriceOverride, setCyclePriceOverride] = useState<
    Record<string, number>
  >({});
  const [priceModal, setPriceModal] = useState(false);

  // overrides locais no modo manual
  const [overrides, setOverrides] = useState<
    Record<string, { allocationPct?: number; preventiveMargin?: number }>
  >({});

  // Em modo automático usamos overrides = margens editadas + % recomendados
  const effectiveOverrides = useMemo(() => {
    if (mode === "manual") return overrides;
    // no auto, aplicamos as margens editadas mas removemos allocationPct para usar as do UB
  ...
  const cycle = energyCreditsProvider.getGeneratingUnitCycleSummary(ugId, {
    month,
    ugId,
  });
  const plan = energyCreditsProvider.getCreditAllocationPlan(
    ugId,
    month,
    effectiveOverrides,
  );
  ...
  // No modo automático substitui o % pelo recomendado para exibir
  const displayRows = plan.rows.map((r) => ({
    ...r,
    displayPct:
      mode === "auto"
        ? r.recommendedAllocationPercentage
  ...
  // valida rateio manual parcial (algum % ausente/zero)
  const partialManual =
    mode === "manual" &&
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
        ...
        <KpiCard
          label="Compensado"
          value={kwh(cycle.totalCompensatedKwh)}
          tone="positive"
        />
        <div
          title="Soma dos créditos acumulados nas Unidades Consumidoras Beneficiárias. Não representa saldo disponível na Unidade Geradora."
        >
          <KpiCard
            label="Saldo total nas UCs"
            value={kwh(cycle.totalFinalBalanceKwh)}
            hint="Soma dos saldos das UCs beneficiárias"
          />
        </div>
        <KpiCard
          label="Beneficiárias"
          value={String(cycle.beneficiariesCount)}
        />
      </div>
      ...
      <Card className="p-5">
        <SectionTitle
          ...
          right={
            <div className="flex items-center gap-2">
              <Badge tone={pctInvalid ? "red" : "green"}>
                Total: {(totalDisplayPct * 100).toFixed(2)}%
              </Badge>
        ...
        {partialManual && (
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50/60 px-3 py-2 text-xs text-amber-800">
            ...
            <div>
              <div className="font-semibold uppercase text-[10px] tracking-wider">
                PARTIAL_MANUAL_ALLOCATION_NOT_ALLOWED
              </div>
      ...
      <Card className="p-5">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wider text-slate-500 border-y border-slate-100 bg-slate-50/50">
                <th className="py-2.5 px-5 font-medium sticky left-0 bg-slate-50/95 z-10">
                  Beneficiária / UC
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="averageMonthlyConsumptionKwh">Média mensal</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="preventiveMarginPercentage">Margem prev.</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="targetCreditKwh">Crédito alvo</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="currentBalanceKwh">Saldo atual</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="recommendedCreditsToReceiveKwh">Recomendado add.</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="allocationPercentage">% Rateio</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="plannedCreditsReceivedKwh">Planejado</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">Δ Rec × Plan</th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="creditsReceivedKwh">Recebido</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="monthlyConsumptionKwh">Consumo</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">
                  <Tt hint="creditsCompensatedKwh">Compensado</Tt>
                </th>
                <th className="py-2.5 font-medium text-right">Saldo final</th>
                <th className="py-2.5 px-5 font-medium text-right">
                  <Tt hint="coverageMonths">Cobertura</Tt>
                </th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={r.ub.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="py-3 text-right tabular-nums">
                      {kwh(r.averageMonthlyConsumptionKwh)}
                    </td>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setOverrides((o) => ({
                            ...
                            [r.ub.id]: {
                              ...o[r.ub.id],
                              preventiveMargin: Number(e.target.value) / 100,
                            },
                          }))
                        }
                    ...
                    <td className="py-3 text-right tabular-nums text-emerald-700 font-medium">
                      {kwh(r.targetCreditKwh)}
                    </td>
                    <td className="py-3 text-right tabular-nums text-slate-700">
                      {kwh(r.currentBalanceKwh)}
                    </td>
                    <td className="py-3 text-right tabular-nums text-slate-700">
                      {kwh(r.recommendedCreditsToReceiveKwh)}
                    </td>
                    <td className="py-3 text-right">
                      {mode === "manual" ? (
                        <>
                          <input
                            ...
                            onChange={(e) =>
                              setOverrides((o) => ({
                                ...
                                [r.ub.id]: {
                                  ...o[r.ub.id],
                                  allocationPct: Number(e.target.value) / 100,
                                },
                              }))
                            }
                    ...
                    <td
                      className={`py-3 text-right tabular-nums text-[11px] ${
                        Math.abs(diff) < 1
                          ? "text-slate-400"
                          : diff > 0
                            ? "text-emerald-600"
                    ...
                    <td className="py-3 text-right tabular-nums">
                      {kwh(r.creditsReceivedKwh)}
                    </td>
                    <td className="py-3 text-right tabular-nums">
                      {kwh(r.monthlyConsumptionKwh)}
                    </td>
                    <td className="py-3 text-right tabular-nums text-emerald-700 font-medium">
                      {kwh(r.creditsCompensatedKwh)}
                    </td>
                    <td className="py-3 text-right tabular-nums font-semibold">
                      {kwh(r.finalBalanceKwh)}
                    </td>
                    <td className="py-3 px-5 text-right">
                      <CoverageBadge months={r.coverageMonths} />
                    </td>
                  </tr>
                );
              })}
              ...
              <tr className="border-t-2 border-slate-200 bg-emerald-50/40 font-semibold">
                ...
                <td
                  className={`py-3 text-right tabular-nums ${
                    pctInvalid ? "text-rose-600" : "text-emerald-700"
                  }`}
                >
                  {(totalDisplayPct * 100).toFixed(2)}%
                </td>
                <td className="py-3 text-right tabular-nums text-emerald-700">
                  {kwh(cycle.totalPlannedKwh)}
                </td>
                <td></td>
                <td className="py-3 text-right tabular-nums">
                  {kwh(cycle.totalReceivedKwh)}
                </td>
                <td className="py-3 text-right tabular-nums">
                  {kwh(plan.totalConsumption)}
                </td>
                <td className="py-3 text-right tabular-nums text-emerald-700">
                  {kwh(cycle.totalCompensatedKwh)}
                </td>
                <td className="py-3 text-right tabular-nums">
                  {kwh(cycle.totalFinalBalanceKwh)}
                </td>
        ...
        <div className="mt-3 flex items-start gap-2 rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-2 text-[11px] text-slate-600">
          ...
          <div>
            O saldo já acumulado na UC é descontado do crédito recomendado para
            evitar acúmulo excessivo. Cobertura ≤ 0,25 mês = Baixa · 0,26–1,50 =
            Adequada · &gt; 1,50 = Elevada.
          </div>
        </div>
      </Card>
      ...
      <Card className="p-5">
        ...
        />
        {(() => {
          const terms = energyCreditsProvider.getGeneratingUnitCommercialTerms(ugId);
          ...
          return (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              ...
              <InfoBlock
                label="Preço aplicado no ciclo"
                value={`R$ ${num(applied, 2)}/kWh`}
              />
      ...
      <Card className="p-5 bg-gradient-to-br from-slate-50 to-emerald-50/50">
        <SectionTitle
          title="Resumo financeiro da apuração"
          desc="Repasse ao proprietário e receita bruta ESA no ciclo"
        />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KpiCard
            label="Preço aplicado"
            value={`R$ ${num(
              cyclePriceOverride[`${ugId}-${month}`] ?? plan.ug.purchasePrice,
              2,
            )}/kWh`}
          />
          <KpiCard
            label="Repasse ao proprietário"
            value={brl(plan.ownerPayment)}
            tone="negative"
          />
          <KpiCard label="Receita ESA" value={brl(plan.esaRevenue)} tone="positive" />
          <KpiCard
            label="Spread bruto"
            value={brl(plan.esaRevenue - plan.ownerPayment)}
            tone="accent"
          />
        </div>
      </Card>
      ...
      {priceModal && (
        <CyclePriceModal
          ...
          onApply={(v) => {
            ...
            toast("Preço aplicado somente neste ciclo", {
              description: "O preço padrão da UG não foi alterado.",
            });
            setPriceModal(false);
          }}
        />
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button onClick={() => onApply(Number(price.replace(",", ".")))}>
            Aplicar somente neste ciclo
          </Button>
        </>
      }
    >
...
function ModeButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex-1 text-left rounded-xl border p-3.5 transition-all ${
        active
    ...
    >
      <div className="flex items-center gap-2">
        <Icon
          className={`h-4 w-4 ${active ? "text-emerald-600" : "text-slate-500"}`}
        />
        <span
