...
function runStaticSuite() {
  ...
  assert('UP35 session-init.js só lê "users" — agora via resolveUserByLogin() (_shared/user-identity.js), que internamente faz db.ref(\'users\').once(\'value\'); a leitura direta foi consolidada lá para resolver uid pela CHAVE do Firebase, nunca pelo campo .uid (correção de missão posterior), e desde a missão de login por e-mail passa o identificador BRUTO (trimmedIdentifier) para permitir login OU e-mail',
    sessionInitSrc.includes("resolveUserByLogin(db, trimmedIdentifier)") &&
